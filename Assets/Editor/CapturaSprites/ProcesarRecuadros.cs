...
public class ProcesarRecuadros : ScriptableObject
{
 ...
 /*  para camara en tiempo real creo 
    ...
    public Mat ProcesarTextura(Texture2D texturaEntrada, bool reciclarMat = false)
    {
        if (texturaEntrada == null) return null;
        if (reciclarMat && matOriginal!=null && ultimaTexturaProcesada==texturaEntrada) return ProcesarTextura(matOriginal);
        ultimaTexturaProcesada = texturaEntrada;
        return ProcesarTextura(OCVUnity.TextureToMat(texturaEntrada));
    }
...
/* para hacer camara en tiempo real creo
